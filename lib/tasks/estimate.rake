...
namespace :estimate do
  ...
  def fetch_locations_for_cvs(zip_code)
    responses = {}
    CVS_NDC_DATA.each do |data|
      data[:ndcInfo].each do |ndc_info|
        ndc = ndc_info[:ndc] 
        cached(["cvs", "locations", zip_code, Date.today.to_s, ndc], data_type: :cvs_locations) do
         JSON.parse(cvs_query(ndc, zip_code))
        end
      end
    end
  end
  ...
  def fetch_locations_for_address(address, retailer:)
    if retailer == :cvs
      fetch_locations_for_cvs(address)
